...
if (existingOtp && (Date.now() - existingOtp.createdAt.getTime()) < process.env.OTP_EXPIRY * 1000) {
  const timeLeft = process.env.OTP_EXPIRY - Math.floor((Date.now() - existingOtp.createdAt.getTime()) / 1000);
  return res.status(400).json({
    message: `OTP already sent! Please wait ${timeLeft} seconds before requesting again.`,
  });
}
